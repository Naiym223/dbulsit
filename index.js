   ...
   if(!uptimerate)
      {
       ...
       client.channels.cache.get("921210058895925258").send(`<@${newPresence.userID}> is Online And Uptime Rate - ${uptimerate}% And was Offline for ${hour}h ${minutes}m ${seconds}s`) 
       db.set(`timefr_${newPresence.userID}`, Date.now())
    }
    }
...
client.on('voiceStateUpdate', async(oldState, newState) => {
  ...
  if (!oldState.channel) {
    ...
    if (dateDiff >= 60000) {
      console.log("Gave 1 coin to user, as they have been in VC for more than 1 minuete.")
    ...
    client.channels.cache.get('921138367679332393').send(`Hey <@${newState.member.id}>, Thanks for being active! You have got some **DBU Coins** for being active!`)
    }
  }
})
...
client.on('guildMemberRemove', async member => {
    ...
    client.guilds.fetch(config.server.id).then(bota => {
      client.channels.cache.get(config.server.channels.botlog).send(`<:deny:921193390614384650> <@${data.ownerID}>'s bot **${bota.username}** has been kicked from server and deleted from website \nReason: Owner left server`)
    });
    ...
    client.guilds.fetch(config.server.id).then(bota => {
      client.channels.cache.get(config.server.channels.botlog).send(`<:deny:921193390614384650> <@${serverfind.ownerID}>'s server **${serverfind.name}** has been deleted from website \nReason: Owner left server`)
    });
...
client.on('ready',async () => {
    console.log("[dbulist.xyz]: Bot successfully connected as "+client.user.tag+".");
    let botsSchema = require("./src/database/models/botlist/bots.js");
    const bots = await botsSchema.find();
    client.user.setPresence({ activity: { type: 'WATCHING', name: 'dbulist.xyz | '+bots.length+' bots' }, status: "online" });
});
