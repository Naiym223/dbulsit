...
fs.readdir("./src/servers/commands/", (err, files) => {
  ...
  console.log(`[dbulist.xyz/servers]: ${files.length} command loaded.`);
...
serverClient.on('ready',async () => {
    console.log("[dbulist.xyz/servers]: Bot successfully connected as "+serverClient.user.tag+".");
    let serversdata = require("../../src/database/models/servers/server.js");
    const servers = await serversdata.find();
    serverClient.user.setPresence({ activity: { type: 'PLAYING', name: '- | '+ servers.length+ ' servers' }, status: "online" });
});
