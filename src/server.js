...
module.exports = async (client) => {
  ...
  app.engine("dbulist.xyz", ejs.renderFile);
  app.set("view engine", "dbulist.xyz");
  ...
  app.get("/robots.txt", function(req, res) {
    ...
    res.send(`Sitemap: https://dbulist.xyz/bot/sitemap.xml`);
  });
  app.get("/sitemap.xml", async function(req, res) {
    let link = "<url><loc>https://dbulist.xyz/bot/</loc></url>";
    let botdataforxml = await botsdata.find()
    botdataforxml.forEach(bot => {
      link += "\n<url><loc>https://dbulist.xyz/bot/" + bot.botID + "</loc></url>";
    })
  ...
  app.get("/callback", passport.authenticate("discord", {
    ...
    client.users.fetch(req.user.id).then(async a => {
      client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
        ...
        dynamic: true
      })).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://dbulist.xyz/user/${a.id}) The user named **site** tried to log in but could not log in because he was blocked from the site.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
    })
    req.session.destroy(() => {
      res.json({
        login: false,
        message: "You can no longer log into our site, because your account is blacklisted/banned.",
        logout: true
      })
      req.logout();
    });
    ...
    client.users.fetch(req.user.id).then(async a => {
      client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
        ...
        dynamic: true
      })).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](https://dbulist.xyz/user/${a.id}) User named **site** logged in.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
...
app.get("/admin/premium/give/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.guilds.fetch(serverdata.id).then(bota => {
    client.channels.cache.get(config.server.channels.botlog).send(`<:approve:921193380288016444> <@${serverdata.ownerID}>'s server **${bota.name}** has been promoted to **Premium**.`)
    client.users.cache.get(serverdata.ownerID).send(`<:approve:921193380288016444> Your server named **${bota.name}** has been promoted to **Premium**.`)
  });
...
app.get("/admin/premium/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.guilds.fetch(serverdata.id).then(bota => {
    client.channels.cache.get(config.server.channels.botlog).send(`<:deny:921193390614384650> <@${serverdata.ownerID}>'s server named **${bota.name}**'s Premium has been removed.`)
    client.users.cache.get(serverdata.ownerID).send(`<:deny:921193390614384650> Your server named **${bota.name}**'s Premium has been removed.`)
  });
...
app.get("/admin/boost/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.users.fetch(botdata.botID).then(bota => {
    client.channels.cache.get(channels.botlog).send(`<:deny:921193390614384650> <@${botdata.ownerID}>'s bot named **${bota.tag}**'s promotion has been removed.`)
    client.users.cache.get(botdata.ownerID).send(`<:deny:921193390614384650> Your bot named **${bota.tag}**'s boost has been removed.`)
  });
  await appsdata.deleteOne({
    botID: req.params.botID
  })
...
app.get("/admin/server/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.channels.cache.get(channels.botlog).send(`<:deny:921193390614384650> <@${botdata.ownerID}>'s server named **${botdata.name}** has been deleted by ${req.user.username}.`)
...
app.get("/admin/promote/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.users.fetch(botdata.botID).then(bota => {
    client.channels.cache.get(channels.botlog).send(`<:deny:921193390614384650> <@${botdata.ownerID}>'s bot named **${bota.tag}**'s promotion has been removed.`)
    client.users.cache.get(botdata.ownerID).send(`<:deny:921193390614384650> Your bot named **${bota.tag}**'s promotion has been removed.`)
  });
  await appsdata.deleteOne({
    botID: req.params.botID
  })
...
app.get("/admin/boost/give/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.users.fetch(botdata.botID).then(bota => {
    client.channels.cache.get(channels.botlog).send(`<:approve:921193380288016444> <@${botdata.ownerID}>'s bot  **${bota.tag}** has been **Boosted**.`)
    client.users.cache.get(botdata.ownerID).send(`<:approve:921193380288016444> Your bot named **${bota.tag}** has been **Boosted**.`)
  });
  let guild = client.guilds.cache.get(config.server.id)
...
app.get("/admin/promote/give/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.users.fetch(botdata.botID).then(bota => {
    client.channels.cache.get(channels.botlog).send(`<:approve:921193380288016444> <@${botdata.ownerID}>'s bot  **${bota.tag}** has been **Promoted**.`)
    client.users.cache.get(botdata.ownerID).send(`<:approve:921193380288016444> Your bot named **${bota.tag}** has been **Promoted**.`)
  });
  let guild = client.guilds.cache.get(config.server.id)
...
app.post("/admin/certificate/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.users.fetch(botdata.botID).then(bota => {
    client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}>'s bot named **${bota.tag}** has not been accepted into the certification program / or was removed from the program by moderator <@${req.user.id}>.`)
    client.users.cache.get(botdata.ownerID).send(`<:deny:921193390614384650> Your bot named **${bota.tag}** certification application has been declined by moderator <@${req.user.id}>.\nReason: **${rBody['reason']}**`)
  });
  await appsdata.deleteOne({
    botID: req.params.botID
  })
