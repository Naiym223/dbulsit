...
console.log("[discordbotuniverse.com]: Admin/Botlist/Decline Bot router loaded.");
...
app.post("/admin/decline/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(botdata.ownerID).then(sahip => {
        client.channels.cache.get(global.config.server.channels.botlog).send(`<:deny:921193390614384650> <@${botdata.ownerID}>'s bot named **${botdata.username}** has been declined.\nReason: **${rBody['reason']}** `)
        client.users.cache.get(botdata.ownerID).send(`<:deny:921193390614384650> Your bot named **${botdata.username}** has been declined.\nReason: **${rBody['reason']}**\nAuthorized: **${req.user.username}**`)
    })
