...
app.get("/admin/confirm/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(req.params.botID).then(bota => {
        client.channels.cache.get(global.config.server.channels.botlog).send(`<:approve:921193380288016444> <@${botdata.ownerID}>'s bot named **${bota.tag}** has been approved by <@${req.user.id}>.`)
        client.users.cache.get(botdata.ownerID).send(`<:approve:921193380288016444> Your bot named **${bota.tag}** has been approved by <@${req.user.id}>.`)
    });
