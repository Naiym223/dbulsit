...
app.post("/admin/certificate/delete/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(botdata.botID).then(bota => {
        client.channels.cache.get(channels.botlog).send(`<:deny:921193390614384650> <@${botdata.ownerID}>'s bot named **${bota.tag}** has not been accepted into the certification program / or was removed from the program by moderator <@${req.user.id}>.`)
