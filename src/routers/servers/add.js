...
console.log("[dbulist.xyz/servers]: Add Server router loaded.");
...
app.post("/add", global.checkAuth, async (req,res) => {
    ...
    guild.fetchInvites().then(async fetchinvite => {
      fetchinvite.array().find(a => a.inviter.id === client.user.id)
        ...
        : await guild.channels.cache.filter(x => x.type == "text").first().createInvite({ maxAge: 0 });
    });
    guild.fetchInvites().then(async fetchinvite => {
    ...
    client.channels.cache.get(global.config.server.channels.botlog).send(`<:db_add:895408144602247168> <@${checkGuilds.ownerID}> added **${guild.name}** \n https://dbulist.xyz/server/${checkGuilds.id}/`)
